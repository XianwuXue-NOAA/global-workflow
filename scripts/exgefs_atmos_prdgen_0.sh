...
for stream in ${PRDGEN_STREAMS}; do
    ...
    case $stream in
        res_2p50)
            ...
            PRDGEN_HOURS="${fhrlst}" #"{0..${FHMAXHF}..${FHOUTLF}} {$(( $FHMAXHF + ${FHOUTLF}))..${fhmax}..${FHOUTLF}}"}
            PRDGEN_SUBMC="prd2p5"
        ...
        res_0p25_s1)
            ...
            PRDGEN_HOURS="${fhrlst}" #"{0..${FHMAXHF}..$(( ${FHOUTHF} * 2 ))}"}
        ...
        res_0p25_s2)
            PRDGEN_GRID="0p25"
            PRDGEN_GRID_SPEC="latlon 0:1440:0.25 90:721:-0.25"
            PRDGEN_HOURS="${fhrlst}" #"{${FHOUTHF}..${FHMAXHF}..$(( ${FHOUTHF} * 2 ))}"
            PRDGEN_SUBMC="prd0p25"
            PRDGEN_A_DIR="pgrb2sp25"
            PRDGEN_A_PREFIX="pgrb2s.0p25."
            PRDGEN_A_LIST_F00="gefs_pgrb2a_f00.parm_0p25"
	...
	if [[ $SENDCOM == "YES" ]]; then
		mkdir -m 775 -p $COMOUT/$COMPONENT/${PRDGEN_A_DIR}
		if [[ ! -z ${PRDGEN_B_DIR} ]]; then
			mkdir -m 775 -p $COMOUT/$COMPONENT/${PRDGEN_B_DIR}
		fi
	fi
